...
@pytest.fixture(scope="module")
def test_user(pyfogies_test_aws_environ: AwsEnviron) -> Iterator[AwsProfile]:
...
def test_create_user_raises_if_exists(test_user: AwsProfile) -> None:
    ...
    with pytest.raises(ValueError, match="already exists"):
        aws_access_key.create_user(username=_TEST_USERNAME)


def test_create_user_creates_initial_key(test_user: AwsProfile) -> None:
    """create_user returns an AwsProfile with credentials and a current key."""
    assert test_user.aws_access_key_id.startswith("AKIA")
    assert len(test_user.aws_secret_access_key) > 0
...
def test_list_users_includes_new_user(test_user: AwsProfile) -> None:
